...
def make_cond_matr(cond_matr, n):
    ...
    for i in range(n_sq):
        k, l = int(i%n), int((i-i%n)/n )
        
        G_matr[i, int((i+1)%n+(i-i%n))] = cond_matr[(2*k+1),l]
        G_matr[i, int((i-1)%n+(i-i%n))] = cond_matr[int((2*k-1)%(2*n)),l]
        G_matr[i, int((i+n)%n_sq)] = cond_matr[(2*k),l]
        G_matr[i, int((i-n)%n_sq)] = cond_matr[(2*k),l-1] 
...
sig_met = 100000
sig_ins = 10
cond_matr = make_conductors(p, n, sig_met, sig_ins)

from matplotlib import pyplot as plt    
cond_matr.shape
plt.figure()
cond_matr[44,10:] = sig_met
cond_matr[45,10:] = sig_met
plt.imshow(cond_matr, cmap='jet')
#plt.imshow(U_arr)

# make_cond_matr(cond_matr, n)

G_matr = make_cond_matr(cond_matr, n)

plt.figure()
plt.imshow(G_matr, cmap='jet')



rho , U_arr = solve_LGS(G_matr, n, 3)
plt.figure()
plt.imshow(U_arr.reshape(n,n), interpolation='bicubic')
#%%
solve_RRN(cond_matr, 0.0001, n, V_0 = 0.0, V_c = 3.0)
3/2551
